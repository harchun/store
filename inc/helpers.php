...
function store_get_tax_names($taxonomy,$post_id=null){
...
function store_has_store_data($post_id=null){
    $id=$post_id ?: get_the_ID();
    foreach(array('store_address','store_hours','store_phone','store_line','store_website','store_price','store_style') as $key){ if(get_post_meta($id,$key,true)) return true; }
    return false;
}
function store_reading_time($post_id=null){
    $content=wp_strip_all_tags(get_post_field('post_content',$post_id ?: get_the_ID()));
    $words=preg_match_all('/\S+/u',$content,$m);
