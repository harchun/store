...
<header class="store-single-header"><div class="store-breadcrumb"><a href="<?php echo esc_url(home_url('/')); ?>">首頁</a> / <?php echo esc_html(store_primary_meta()); ?></div><h1><?php the_title(); ?></h1><div class="store-taxonomies"><?php foreach(array_merge(store_get_tax_names('store_city'),store_get_tax_names('store_type')) as $term): ?><span><?php echo esc_html($term); ?></span><?php endforeach; ?><span><?php echo esc_html(store_reading_time()); ?> 分鐘閱讀</span></div></header>
<?php if(has_post_thumbnail()): ?><div class="store-cover"><?php the_post_thumbnail('full',array('fetchpriority'=>'high')); ?></div><?php endif; ?>
<div class="store-layout"><div class="store-content">
